...
- `src/main/java/.../runtime/ImporterApplication.java`
  ...
  - Accepts `--federation`, `--workflow`, `--base-folder`, `--season` arguments.
...
- `src/main/resources/application.yml`
  ...
  - DB connection driven by `DB_TTLEAGUEDATA_JDBC_URL`, `DB_TTLEAGUEDATA_CREDENTIAL_USERNAME`, `DB_TTLEAGUEDATA_CREDENTIAL_PASSWORD`.
- `deploy-to-wsl.ps1` *(repo root)*
  - PowerShell script: builds the fat JAR via `mvn clean package` and deploys it to a local WSL Debian instance.
- `run-importer.sh` *(repo root, copied to WSL by deploy script)*
  - Bash runner: loads `.env`, exports DB env vars, locates JAR, and executes it.
...
1. Add/update config keys in `application.yml`.
...
5. To deploy and run: execute `.\deploy-to-wsl.ps1` from Windows, then call `~/tt-data-league-importer/run-importer.sh` from inside WSL.
